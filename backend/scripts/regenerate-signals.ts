...
async function regenerateSignals(email: string) {
  ...
  try {
    ...
    for (const windowDays of [30, 180] as const) {
      ...
      if (subscriptionSignal.recurring_merchants.length > 0) {
        console.log(`  ✓ Merchants: ${subscriptionSignal.recurring_merchants.join(', ')}`);
      }
